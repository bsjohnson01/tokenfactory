...
func (k Keeper) SetDenom(ctx context.Context, denom types.Denom) {
	...
	store.Set(types.DenomKey(denom.Denom), b)
}

// GetDenom returns a denom from its index
func (k Keeper) GetDenom(ctx context.Context, denom string) (val types.Denom, found bool) {
	storeAdapter := runtime.KVStoreAdapter(k.storeService.OpenKVStore(ctx))
	store := prefix.NewStore(storeAdapter, types.KeyPrefix(types.DenomKeyPrefix))

	b := store.Get(types.DenomKey(denom))
	if b == nil {
		return val, false
	}
